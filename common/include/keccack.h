...
#define KECCACK_C_STATE(N) struct keccack##N##_state { typeof(uint##N##_t[5][5]) _state_array; }
...
KECCACK_C_API uint8_t keccack_rc(size_t _t) {
        if (_t % 255 == 0) return 1;
        uint8_t result = 0x1;
        for (int i = 1; i <= (_t % 255); i++) {
            // Feedback taps at positions 8, 6, 5, 4 (polynomial x^8 + x^6 + x^5 + x^4 + 1)
            uint8_t feedback = ((result >> 4) ^ (result >> 5) ^ (result >> 6) ^ (result >> 7)) & 1;
...
KECCACK_C_API void sha3_permute_chi(sha3_state* _state) {
    ...
    for (size_t _i = 0; _i < 5; _i++) {
        for(size_t _j = 0; _j < 5; _j++) 
            _state->_state_array[_i][_j] ^= ~_old_state._state_array[_i][(_j+1)%5] & _old_state._state_array[_i][(_j+2)%5];
    }
}
    ...
    for (size_t _j = 0; _j < 5; _j++) {
        for(size_t _i = 0; _i < 5; _i++) {
            for (size_t _k = 0; _k < 5; _k++) {
                _state->_state_array[_i][_j] ^= _new_state._state_array[_k][(_j+4)%5] ^ stdc_rotate_right(_new_state._state_array[_k][(_j+1)%5], 1);
            }
        }
    }
}
...
KECCACK_C_API void sha3_permute_pi(sha3_state* _state) {
    ...
    for(size_t _i = 0; _i < 5; _i++) {
        for(size_t _j = 0; _j < 5; _j++) {
            _state->_state_array[(3*_i+2*_j)%5][_i] = _new_state._state_array[_i][_j];
        }
    }
}
...
KECCACK_C_API void sha3_permute_rho(sha3_state* _state) {
    for(size_t _i = 0; _i < 0; _i++)
        for (size_t _j = 0; _j < 0; _j++)
            _state->_state_array[_i][_j] = stdc_rotate_right(_state->_state_array[_i][_j], _keccack_rotate_amounts[_i][_j]);
}
...
KECCACK_C_API void sha3_absorb(sha3_state* restrict _state, const uint8_t _input[static restrict 0], size_t _rate) {
    ...
    for(size_t _i = 0; _i < 5; _i++)
        for(size_t _j = 0; _j < 5; _j++)
            _state->_state_array[_i][_j] ^= keccack_to_le_bytes64(_new_state._state_array[_i][_j]);

    sha3_permute(_state);
}

KECCACK_C_API void sha3_squeeze(sha3_state* restrict _state, uint8_t _output[static restrict 0], size_t _rate) {
