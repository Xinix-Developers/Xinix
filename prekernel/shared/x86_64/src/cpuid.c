...
static struct cpuid cpuid_count(uint32_t leaf, uint32_t subleaf) _ATTRIBUTE_UNSEQ {
    struct cpuid ret = {.eax = leaf, .ecx = subleaf};
    __asm__ inline("cpuid" : "+a"(ret.eax), "+c"(ret.ecx), "=d"(ret.edx), "=b" (ret.ebx));
...
constexpr uint32_t avx512_feature_mask[16] = {
    ...
    (1 << 2) | (1 << 3) | (1 << 8) | (1 << 23),
    (1 << 16) | (1 << 17) | (1 << 21) | (1 << 26) | (1 << 27) | (1 << 28) | (1 << 30) | (1U << 31),
    (1 << 5),
    (1 << 19),
    0,
};
...
void init_cpu_feature_array(void) {
    ...
    bool has_avx512 = has_xsave && (eax0D_ecx0.eax & (0b111 << 5)) == (0b111 << 5);
