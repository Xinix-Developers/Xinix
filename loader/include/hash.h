...
static inline uint32_t gnu_hash_get_bucket(const struct dt_gnu_hash* hash, uint32_t bucket) {
    auto* arr = &hash->tail[hash->bloom_size];
    return arr[bucket >> HASH_GNU_IMPL_SHIFT].bucket_or_chain[bucket & HASH_GNU_IMPL_MASK];
}

static inline uint32_t gnu_hash_get_chain(const struct dt_gnu_hash* hash, uint32_t chain) {
