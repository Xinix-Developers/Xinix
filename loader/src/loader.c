...
sysresult2_t loader_map_elf(const ElfNative_Ehdr *e_hdr, ElfNative_Dyn** dyn_out, ElfNative_Phdr** phdr_out) {
    ...
    ElfNative_Phdr* phdrs = (ElfNative_Phdr*)((char*)e_hdr) + (e_hdr->e_phoff);
    ...
    if(dyn_out && dyn_segment) {
        *dyn_out = launder_pointer((ElfNative_Dyn*)(((char*)root) + dyn_segment->p_paddr));
    }

    if(phdr_out && phdr_segment) {
        *phdr_out = launder_pointer((ElfNative_Phdr*)(((char*)root) + phdr_segment->p_paddr));
    }
