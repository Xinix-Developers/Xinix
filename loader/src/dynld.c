...
struct DynldState {
    ...
    struct DynLibraryEntry entries[(max_size / sizeof(struct DynLibraryEntry)) - 1];
};

struct DynldState dynld_state;

sysresult_t dynld_link(ElfNative_Dyn dyn[], ElfNative_Phdr* phdrs, size_t phnum, const char* file_name) {
